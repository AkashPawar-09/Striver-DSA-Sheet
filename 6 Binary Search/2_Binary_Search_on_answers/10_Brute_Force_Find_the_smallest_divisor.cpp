...
public:
    int smallestDivisor(vector<int>& nums, int threshold) {
        ...
        for (int k = 1; k <= maxi; k++) {                           // loop from k=1 to maximum element in array

            int divSums = 0;                                        // initiate divSum = 0

            for (int num : nums) {                                  // foreach loop to calculate divSum
                divSums = divSums + (num + k - 1) / k;              // calculate divSum
            }

            if (divSums <= threshold) {                             // if divSum is smaller than or equal to threshold
                return k;                                           // then return it 
                break ;                                             // break the iteration
            }
        }

        return maxi;                                                // otherwise return maximum element
    }
};
